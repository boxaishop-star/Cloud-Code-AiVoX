...
export class InMemoryStore {
  ...
  getFoundation(tenantId: string): BusinessFoundation | undefined {
    return this.foundations.get(tenantId);
  }
  getProductCards(tenantId: string): ProductCard[] {
    return this.productCards.get(tenantId) ?? [];
  }
  getRelationshipCards(tenantId: string): Record<string, unknown>[] {
    return this.relationshipCards.get(tenantId) ?? [];
  }

  applyAction(action: ToolAction): ToolActionResult {
    try {
      switch (action.type) {
        ...
        default:
          ...
          return { action, applied: false, error: `Action "${action.type}" не реализован на Этапе 0` };
      }
    } catch (e) {
