...
describe("Изоляция данных между тенантами (раздел 9 ТЗ)", () => {
  it("ProductCard тенанта A не виден тенанту B", async () => {
    ...
    expect(store.getProductCards("tenant_A")).toHaveLength(1);
    expect(store.getProductCards("tenant_B")).toHaveLength(0); // не должно утечь
  });

  it("BusinessFoundation одного тенанта не подмешивается в другой", async () => {
    const store = new InMemoryStore();
    store.applyAction({ type: "upsert_business_foundation", payload: { tenant_id: "tenant_A", company_description: "Бизнес А" } });
    store.applyAction({ type: "upsert_business_foundation", payload: { tenant_id: "tenant_B", company_description: "Бизнес Б" } });

    expect(store.getFoundation("tenant_A")?.company_description).toBe("Бизнес А");
    expect(store.getFoundation("tenant_B")?.company_description).toBe("Бизнес Б");
  });
});
