...
function buildProfileSetupPrompt(
  ...
  if (context.foundationComplete === false) {
    lines.push(
      ...
      '  • company_name        — brand/company name if explicitly stated (e.g. "Мы — СтройМонолит")',
      ...
      '  • address             — physical address if mentioned (capture only, do NOT ask)',
      '  • phone               — phone number if mentioned (capture only, do NOT ask)',
      '  • working_hours       — working hours if mentioned, e.g. "пн-пт 9:00–18:00" (capture only, do NOT ask)',
      '',
      '## DO NOT turn this into a form (раздел 5 ТЗ)',
      'Never ask for company_name, address, phone, or working_hours — capture them ONLY if',
      'the user mentions them spontaneously. If the user has moved on to describing their',
      'service, do NOT go back to ask for missing optional fields.',
      '',
      ...
      '  (company_name / address / phone / working_hours: capture if mentioned, NEVER ask)',
      '',
  ...
  lines.push(
    ...
    '   or any of: company_name, address, phone, working_hours.',
